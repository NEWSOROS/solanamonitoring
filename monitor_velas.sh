...
#####    velas Validator Monitoring Script v.0.14 to be used with Telegraf / Grafana / InfluxDB
#####    Fetching data from velas validators, outputs metrics in Influx Line Protocol on stdout
#####    Created: 14 Jan 18:28 CET 2021 by Stakeconomy.com. Forked from original Zabbix nodemonitor.sh script created by Stakezone
#####    For support post your questions in the #monitoring channel in the velas discord server
. /home/vlx/service-env.sh
#####    CONFIG    ##################################################################################################
configDir="$HOME/.config/velas" # the directory for the config files, eg.: /home/user/.config/velas
##### optional:        #
...
binDir=""              # auto detection of the velas binary directory can fail or an alternative custom installation is preferred, in case insert like $HOME/velas/target/release
...
if [ -n  "$binDir" ]; then
   ...
   if [ -n "$installDir" ]; then cli="${installDir}/velas"; else echo "please configure the cli manually or check the configDir setting"; exit 1; fi
fi

if [ -z $rpcURL ]; then
   rpcPort=$(ps aux | grep velas-validator | grep -Po "\-\-rpc\-port\s+\K[0-9]+")
   if [ -z $rpcPort ]; then echo "nodemonitor,pubkey=$identityPubkey status=4 $now"; exit 1; fi
   rpcURL="http://127.0.0.1:$rpcPort"
fi

noVoting=$(ps aux | grep velas-validator | grep -c "\-\-no\-voting")
...
velasPrice=$(curl -s 'https://api.hitbtc.com/api/3/public/price/rate?from=vlx&to=usdt' | jq -r '.VLX | .price')
...
if [ $(grep -c $voteAccount <<< $validatorCheck) == 0  ]; then echo "validator not found in set"; exit 1; fi
    ...
    delinquentValidatorInfo=$(jq -r '.currentValidators[] | select(.voteAccountPubkey == '\"$voteAccount\"' and .delinquent == true)' <<<$validators)
    if [[ ((-n "$currentValidatorInfo" || "$delinquentValidatorInfo" ))  ]] || [[ ("$validatorBlockTimeTest" -eq "1" ) ]]; then
        ...
        if [ "$format" == "SOL" ]; then activatedStake=$(echo "scale=2 ; $activatedStake / 1000000000.0" | bc); fi
              ...
              logentry="$logentry,version=\"$version\",pctNewerVersions=$pctNewerVersions,commission=$commission,activatedStake=$activatedStake,credits=$credits,solanaPrice=$velasPrice"
           else status=2; fi
